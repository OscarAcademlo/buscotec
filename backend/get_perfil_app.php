...
{
    $stmt = $conn->prepare(
        "SELECT id, nombre, apellido, email, whatsapp, direccion, barrio, ciudad, provincia, verificado, foto
...
$fmtFoto = function (string $p, bool $isPro): string {
    ...
    return 'https://buscotec.click/img/' . ($isPro ? 'profesionales' : 'usuarios') . '/' . $c;
};
