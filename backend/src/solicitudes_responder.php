...
try {
  ...
  if ($estadoNuevo === 'aceptado') {
    ...
    $stmt2 = $conn->prepare("
        ...
        aceptado_por, estado, accepted_at, pagado, cargo_ars
      )
      VALUES (?, ?, ?, ?, ?, ?, 'profesional', 'aceptado', NOW(), ?, ?)
      ON DUPLICATE KEY UPDATE estado='aceptado', aceptado_por='profesional', accepted_at=NOW(), cargo_ars=VALUES(cargo_ars)
    ");
    
    // Obtener valor ARS actual para congelarlo en el caso
    $resV = $conn->query("SELECT valor FROM ajustes WHERE clave='valor_caso' LIMIT 1");
    $cargoArs = 1400.00; 
    if($resV && $rv = $resV->fetch_assoc()) $cargoArs = (float)$rv['valor'];

    $solicitado_por = $usuario_tipo;
    $stmt2->bind_param("iissssid", $mensajeId, $usuario_id, $usuario_tipo, $prof_id, $prof_tipo, $solicitado_por, $pagado_state, $cargoArs);
    $stmt2->execute();
    $stmt2->close();
    ...
    // 📝 TRAZABILIDAD: Registro de aceptación
    if ($caso_id > 0) {
        $stT = $conn->prepare("INSERT INTO trazabilidad_casos (caso_id, usuario_id, profesional_id, accion, valor_nuevo) VALUES (?, ?, ?, 'aceptado', ?)");
        $stT->bind_param("iiid", $caso_id, $usuario_id, $prof_id, $cargoArs);
        $stT->execute();
        $stT->close();
    }
