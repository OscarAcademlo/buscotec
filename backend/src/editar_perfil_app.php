...
if ($userId) {
    $stmt = $conn->prepare(
        "UPDATE usuarios SET nombre=?, apellido=?, email=?, whatsapp=?, direccion=?, barrio=?, ciudad=?, provincia=?, fecha_nacimiento=? WHERE id=? LIMIT 1"
    );
    if ($stmt) {
        $stmt->bind_param('sssssssssi', $nombre, $apellido, $email, $whatsapp, $direccion, $barrio, $ciudad, $provincia, $fecha_nacimiento, $userId);
        if (!$stmt->execute()) {
            $success = false;
...
if ($profId) {
    ...
    $sql = "UPDATE profesionales SET nombre=?, apellido=?, email=?, whatsapp=?, direccion=?, barrio=?, ciudad=?, provincia=?, fecha_nacimiento=?";
    $params = [$nombre, $apellido, $email, $whatsapp, $direccion, $barrio, $ciudad, $provincia, $fecha_nacimiento];
    $types = "sssssssss";
