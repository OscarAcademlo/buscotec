...
try {
    ...
    // Si la acción es eliminar o desactivar ubicación
    if ($action === 'delete') {
        $stmtDel = $db->prepare("DELETE FROM ubicaciones_usuarios WHERE user_id = ? AND rol = ?");
        $stmtDel->bind_param('is', $user_id, $rol);
        $stmtDel->execute();
        $stmtDel->close();

        if ($rol === 'profesional') {
            $stmtUp = $db->prepare("UPDATE profesionales SET lat = NULL, lng = NULL WHERE id = ?");
            $stmtUp->bind_param('i', $user_id);
            $stmtUp->execute();
            $stmtUp->close();
        }
        respond(true, 'Ubicación eliminada correctamente');
    }
